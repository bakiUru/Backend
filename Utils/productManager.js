...
class ProductManager {
  ...
  async productAdd(
    ...
    if (this.products.length > 0) {
      ...
      if (this.products.find((product) => product.code === code))
        return {
          error: `El Codigo del Producto ya existe --->  ${code}  NO SE INGRESO EL PRODUCTO: ${prod.title}`,
        };
      else {
  ...
  async deleteProduct({ data }, id) {
    ...
    try {
      if (product.length == 0)
        return {
          messageError: `Producto no encontrado para Eliminar: ID ${id}`,
          status: 400,
        };
      else {
  ...
  async putProduct(
    ...
    try {
      if (product.length == 0)
        return {
          messageError: `Producto no encontrado para Modificar: ID ${id}`,
          status: 400,
        };
      else {
        //filtro todos los productos que no coinciden con ese array
        ...
        product.category = category == null ? product.category : category;
        product.code = code == null ? product.code : code;
  ...
  idGeneration(list) {
    ...
    if (list.length == 0) return 1; //si es el primer producto retorno 1
    else {
      //obtengo solo los numero de ID
      list.map((prod) =>
        Object.entries(prod).forEach(([key, val]) => {
          if (key === "id") newlist.push(val);
        })
      );
      ...
      if (Math.max(...newlist) > newlist.length) {
        ...
        if (Math.min(...newlist) - 1 === 1) return 1;
        else {
          newlist.forEach((num) => {
            if (newId == num) newId++;
          });
          console.log("Nuevo ID:", newId);
          return newlist.find((id) => id === newId)
            ? Math.max(...newlist) + 1
            : newId;
        }
      } else return newlist.length + 1;
    }
  }
}
